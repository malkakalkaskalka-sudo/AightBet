...
(function () {
  ...
  function initParticles() {
    ...
    function drawSun(t) {
      ...
      for(let i=0;i<NUM_RAYS;i++){ const angle=(i/NUM_RAYS)*Math.PI*2; const rayLen=(i%2===0?R*2.0:R*1.4)*pulse; const rayW=i%2===0?3.5:2; const alpha=LM?0.55:0.70; ctx.save();ctx.rotate(angle);
        const rg=ctx.createLinearGradient(R*0.9,0,R*0.9+rayLen,0); rg.addColorStop(0,`rgba(255,230,80,${alpha})`); rg.addColorStop(0.5,`rgba(255,170,40,${alpha*0.6})`); rg.addColorStop(1,'rgba(255,120,30,0)');
        ctx.strokeStyle=rg;ctx.lineWidth=rayW;ctx.lineCap='round'; ctx.beginPath();ctx.moveTo(R*0.95,0);ctx.lineTo(R*0.95+rayLen,0);ctx.stroke();ctx.restore();
      }
      ctx.restore();
  ...
  function mountSunButton() {
    ...
    /* FIX: bump z-index to max so the (invisible) button sits ABOVE the
       navbar / support-fabs / mythic banner on PC. Previously z-index:9500
       was below the navbar on desktop so clicks were intercepted and the
       button "disappeared". Now it's clickable everywhere (still hidden). */
    s.textContent = `
      #ck-sun-btn{
        position:fixed;
        top:14px;
      ...
      #ck-sun-btn:hover{transform:scale(1.06);}
      #ck-sun-btn:active{transform:scale(.94);}
      /* Hide on small landscape mobile (matches the rest of the widget) */
      @media (orientation:landscape) and (max-width:1024px){
        #ck-sun-btn{display:none!important;}
      }
    `;
    document.head.appendChild(s);
